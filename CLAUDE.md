...
├── child-theme/               # The JNews child theme (Git-tracked source)
│   ├── style.css
│   ├── functions.php
│   ├── screenshot.png
│   └── ...
...
The `child-theme/` folder is symlinked into Local's WordPress install:

```
~/Local Sites/enjoyhr/app/public/wp-content/themes/jnews-child  →  ~/claude-projects/enjoy-hr/child-theme
```

You edit the source in `~/claude-projects/enjoy-hr/child-theme/` and WordPress reads it through the symlink.
...
1. **Develop locally** in Local by Flywheel — edit files in `~/claude-projects/enjoy-hr/child-theme/`, view at `http://enjoyhr.local`
